...
function CourseVideos() {
  ...
  const handleClosetask = () => setShow(false);
  const handleShowtask = () => setShow(true);
    ...
    <>
      <div className="container-fluid">
        <div className="row">
            ...
            >
              ...
              {sidebarItems.map((item, index) => {
                ...
                const formattedTitle =
                  title.length > 11 ? (
                    <>
                      <h6>{title.slice(0, 12)} <br /> {title.slice(12)}</h6>
              ...
              <Button variant="success"  className="navpart px-3 my-5 ">
              <FontAwesomeIcon icon={faCamera}></FontAwesomeIcon> Live Class
              </Button>
              <Button variant="primary" onClick={handleShowtask} className="navpart px-3">
                Tech Task
              </Button>
              <Modal show={show} onHide={handleClosetask} centered>
                  <Modal.Header closeButton>
                    <Modal.Title>Task Information</Modal.Title>
                  </Modal.Header>
                  <Modal.Body>
                    <b>Hi learner, in this task we will access your mic and camera to record your audio and video. Please accept this to continue.</b>
                  </Modal.Body>
                  <Modal.Footer>
                    <Button variant="secondary" onClick={handleClosetask}>
                      Close
                    </Button>
                    <Link to={`/techtask/${id}`}  className=" btn btn-success navpart px-3">Start Task</Link>
                  </Modal.Footer>
                </Modal>
              {/* <Link to={`/techtask/${id}`}  className="navpart px-3">Tech Task</Link> */}
          ...
          <div
            className={`col ${
              isSidebarOpen ? "col-sm-12 col-lg-8" : "col-sm-12 col-lg-8"
