...
function Lessons() {
  ...
  const [hasPaid, setHasPaid] = useState(true); // State for payment status
  ...
  useEffect(() => {
    ...
    // Fetch module data
    axios
      .get(`${process.env.REACT_APP_API_URL}course/getmodule/3`)
      .then((res) => {
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then((res) => {
        ...
        setHasPaid(true); // Set the payment status
      })
      .catch((err) => {
        console.log(err);
      });
